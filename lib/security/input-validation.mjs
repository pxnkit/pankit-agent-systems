export const CHAT_INPUT_LIMITS = Object.freeze({
  ...
  historyItems: 6,
  historyCharacters: 4_200,
  turnstileTokenCharacters: 2_048,
  sessionIdCharacters: 80,
});
...
export function validateChatPayload(payload) {
  ...
  for (const [index, entry] of historyInput.entries()) {
    ...
    if (normalized.length > CHAT_INPUT_LIMITS.messageCharacters) {
      return {
        ok: false,
        error: `history[${index}].content must be at most ${CHAT_INPUT_LIMITS.messageCharacters} characters.`,
      };
    }
  ...
  if (value.sessionId !== undefined) {
    if (
      ...
      value.sessionId.length > CHAT_INPUT_LIMITS.sessionIdCharacters ||
      !/^[a-z0-9][a-z0-9._:-]{7,79}$/i.test(value.sessionId)
    ) {
