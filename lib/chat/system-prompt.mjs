...
export function buildSystemPrompt(sources) {
  ...
  const evidence = selectedSources
    ...
    .join("\n");
  const citationContract = selectedSources
    .map((source) => `[source:${escapeEvidence(source.id)}]`)
    .join(", ");
...
- Return one or two concise paragraphs only: no heading, preamble, source list, or markdown link.
- Copy a citation exactly from this allowlist; do not add spaces or alter an ID: ${citationContract || "(no citations available)"}.
