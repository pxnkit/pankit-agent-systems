...
export function providerFallbackReason(error) {
  ...
  if (/did not return a readable stream/.test(message)) {
    return "provider-stream-unavailable";
  }
  if (/invalid stream event/.test(message)) {
    return "provider-stream-invalid";
  }
  if (/grounding validation|valid grounding|no grounded answer/.test(message)) {
    return "provider-grounding-failed";
  }
  if (
    status === 429 ||
